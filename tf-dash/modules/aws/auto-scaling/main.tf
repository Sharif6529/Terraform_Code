...
resource "aws_autoscaling_policy" "network_high" {
  name                   = "${var.name_prefix}-network-high"
  autoscaling_group_name = aws_autoscaling_group.this.name
  adjustment_type        = "ChangeInCapacity"
  policy_type            = "SimpleScaling"
  scaling_adjustment     = "1"
...
resource "aws_autoscaling_policy" "network_low" {
  name                   = "${var.name_prefix}-network-high"
...
resource "aws_cloudwatch_metric_alarm" "network-high" {
  alarm_name          = "${var.name_prefix}-network-high-alarm"
